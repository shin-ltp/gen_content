"""Re-fetch apple-hq / coreweave-hq avoiding watermark sources (vecteezy etc.)."""
import json
import re
import time
...
BAD = (
    ...
    "cleanpng", "seeklogo", "worldvectorlogo", "svgrepo", "wikia", "fandom",
    "ytimg", "substackcdn", "medium.com", "twitter", "reddit", "instagram",
)
...
JOBS = {
    "apple-hq.jpg": [
        ...
        "Apple headquarters Apple Way Cupertino sign",
    ],
    "coreweave-hq.jpg": [
        "CoreWeave headquarters Livingston New Jersey office",
        "CoreWeave building exterior logo",
    ],
}
    ...
    for dest_name, queries in JOBS.items():
        ...
        for q in queries:
            ...
            for r in data.get("results", []):
                ...
                if good:
                    ok = True
                    with open(BRANDS / "_sources.log", "a", encoding="utf-8") as f:
                        f.write(f"{dest_name} | {src} | {page} | retry-2026-08-23\n")
                    break
                time.sleep(1)
            if ok:
                break
