...
1. 市場データ収集（MKT）
   ...
   - **セクターETF（SPDR 11セクター）の前日騰落**を必ず含む（A-02 市況総括の値上がり/値下がりセクターリストの一次データ。方向判定ができれば足りる・2026-08-31 追加）
2. マクロ収集（MAC）
...
6. 索引作成 + Gate #0 自動検査（2026-08-18 自動化）
   ...
   - **収集器構成メモ（2026-08-19）**:
     - `market`（Yahoo Finance 公共API・認証不要）: MKT スナップ + **SECTOR（SPDR 11セクターETF騰落）** + MAC（米10年/2年金利）+ STK（主要銘柄）を供給
...
> 「多く取って漏らさない」原則により、**量の超過は不合格理由にならない**。不合格は「漏れ」または「形式不備」のみ。ただし Tier B データは DB キャッシュで供給するため、検査8 は「当日分として fresh か」のみを問う。
>
